...
# goes through a password generated here, which a re-run replaces.
...
ask "Install and enable an MPD server (port $MPD_PORT, password-protected)? [y/N]"
read -rp "Choice: " mpd_choice
...
case "${mpd_choice,,}" in
  y|yes)
    ...
    if [ ${#mpd_pkgs[@]} -gt 0 ]; then
      print_info "Installing ${mpd_pkgs[*]} with: $PKG_MANAGER"
      $PKG_MANAGER "${mpd_pkgs[@]}"
    fi
    ...
    cat > "$MPD_CONF" <<EOF
$MPD_MARKER — re-running it replaces this file and the password.
...
EOF
    ...
    ;;
  *)
    # Only undo a server this script set up; a hand-written mpd.conf is left alone.
    if [ -f "$MPD_CONF" ] && head -n1 "$MPD_CONF" | grep -qF "$MPD_MARKER"; then
      ...
      print_ok "Stopped and disabled the MPD server set up by a previous run ($MPD_CONF kept)"
    fi
    ;;
esac
